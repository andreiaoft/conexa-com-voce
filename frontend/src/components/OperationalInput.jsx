...
function UploadIcon() {
  return (
    ...
    >
      <path
        fillRule="evenodd"
        d="M9.25 4.75a.75.75 0 011.5 0v8.69l1.97-1.97a.75.75 0 111.06 1.06l-3.25 3.25a.75.75 0 01-1.06 0L6.22 12.53a.75.75 0 111.06-1.06l1.97 1.97V4.75z"
        clipRule="evenodd"
      />
...
function OperationalInput({ onPlanReceived, onStartNewPlan }) {
  ...
  const {
    ...
    error: planError,
    run: runPlan,
  } = useAsyncAction("Falha na geração do plano.");

  const {
    loading: pdfLoading,
    error: pdfError,
    run: runPdf,
  } = useAsyncAction("Erro ao processar o PDF.");
  ...
  async function handleSubmit(e) {
    ...
    // limpa só a visualização da direita
    onStartNewPlan();
  ...
  return (
    <Card>
      <h2 className="mb-3 text-lg font-semibold text-conexa-blue-dark">
        Entrada operacional
      </h2>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <label
            className={`inline-flex cursor-pointer items-center gap-1.5 rounded border border-conexa-blue-medium px-3 py-1.5 text-xs font-medium text-conexa-blue-medium transition-colors duration-150 hover:bg-conexa-blue-light ${
...
OperationalInput.propTypes = {
  ...
  onStartNewPlan: PropTypes.func.isRequired,
};
